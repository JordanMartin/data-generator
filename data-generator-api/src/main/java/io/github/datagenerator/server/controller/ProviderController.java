...
import io.github.datagenerator.domain.definition.DefinitionException;
import io.github.datagenerator.server.domain.FormOutputConfig;
import io.github.datagenerator.server.domain.ProviderConf;
import io.github.datagenerator.server.repository.ProviderRepository;
import io.github.datagenerator.server.utils.ProviderDownloadUtil;
import io.github.datagenerator.server.utils.SleepUtil;
...
//                                   String definition) {
//        FormOutputConfig outputConfig = new FormOutputConfig(Map.of(
//                "pretty", "true",
//                "format", format
//        ));
...
//    }
    ...
    public Response liveProvider(@FormParam("definition") String definition,
                                 @BeanParam FormOutputConfig outputConfig) {
    ...
    @Path("/download")
    @Consumes(MediaType.APPLICATION_FORM_URLENCODED)
    public Response download(@BeanParam FormOutputConfig outputConfig, @FormParam("definition") String definition) {
        ProviderConf providerConf = ProviderConf.from(definition, outputConfig);
        if (outputConfig.getRequiredBoolean("zip")) {
            if (outputConfig.getRequiredInteger("count_per_file") <= 1) {
        ...
        } else if (outputConfig.getRequiredBoolean("gzip")) {
